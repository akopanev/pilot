...
```
.pilot/
├── pipeline.yaml          # pipeline config (committed)
├── prompts/               # prompt templates (committed)
│   ├── implement.md
│   ├── review.md
│   └── fix.md
├── agents/                # review agents (committed, customizable)
│   ├── quality.md
│   ├── implementation.md
│   ├── testing.md
│   ├── simplification.md
│   └── documentation.md
├── protocol.md            # signal protocol reference (committed)
├── README.md              # full spec reference (committed)
└── session/               # per-run state (gitignored)
    ├── tasks/             # task files for iterator loops
    ├── session.json       # step completion tracking
    ├── emissions/         # emitted key-value pairs
    ├── qa/                # Q&A pairs from question signals
    └── progress-*.log     # execution logs
```

**Gitignore:** Add `.pilot/session/` to your `.gitignore`. Everything else in `.pilot/` is project config and should be committed.
...
Scaffolds a new project from a built-in pipeline template. Copies pipeline config, prompts, agents, and protocol into `.pilot/`. Creates `.pilot/session/tasks/` for task files. Never overwrites existing files.
